...
- [x] 真实生图已跑通（芦笋牛排全套 6 张，72px 自检通过）
...
- [x] 服务端自动抓链接提炼 → R9 已实现（抖音扒文案+og兜底）
...
- [x] 插画生成已上线（App 内一键，芦笋牛排已出全套；其余菜待 zzf 按需点按钮）
...
## R13 食材小百科 ✅（2026-07-19）

- [x] 知识库：`data/ingredients/<名>.json`，AI 生成一次全食单缓存复用（走 claude 订阅零 API 费）；数值按《中国食物成分表》口径 + UI 免责脚注；修复过 prompt 示例数字锚定问题
- [x] `GET /api/ingredient/{name}`：缓存命中 20ms，首查 ~12s 生成
- [x] 教程卡食材可点 → 底部小百科弹卡：每100g 热量/蛋白/脂肪/碳水四宫格 + 营养功效 2-3 条 + 挑选/储存小贴士 + 本菜用量
- [x] 存量 36 个食材已预生成（含调料：生抽/八角/料酒也有条目）

## R14 营养数据库 ✅（2026-07-19）

- [x] 内置营养库 `server/assets/nutrition-db.json`：69 种常用食材精编（《中国食物成分表(第6版)》常见参考值口径），含每100g四大营养素 + 默认克重，仓库文件可维护可校正
- [x] 三级来源体系：内置库（标成分表来源）→ 用户缓存 → AI 兜底（标"AI 估算"）；小百科弹卡脚注显示具体来源
- [x] 编辑器食材结构化行：名称可从 76 项名单选（datalist），选中自动带默认克重，用量/克重可改
- [x] 按克重自动算营养：菜谱详情显示「按食材合计 ≈Nkcal · 蛋白/脂肪/碳水（x/y 项计入）」；AI 整理也自动估每项克数
