...
- 忠实整理，不要发明原文没有的食材和步骤
- **先判断原文是否真的包含可执行步骤**（先做什么、后做什么、火候/时长这类）。
  很多社交媒体链接抓到的只是标题/卖点文案/话题标签（比如只有菜名、"绝绝子""好吃"这类感叹、
  "#减脂餐"这类话题标签），没有任何步骤性描述——这种情况下 ingredients/steps 仍可以按你对
  这道菜的常识给一份合理参考（对用户仍有用），但 **tips 第一条必须用「⚠️」开头**明确写清楚：
  "原文只是标题文案，没有具体做法——以下是参考做法，不是这条视频/文章演示的做法，建议对照
  原视频修正"。如果原文确实包含步骤性描述（哪怕简略、口语化），忠实整理即可，不加这条警示。
...
def ask(prompt: str, timeout: int = 60, json_mode: bool = False) -> str:
    """按生效通道问一句，返回模型原始文本输出。

    timeout 仅作用于 openai 兼容通道；CLI 通道固定 180s（本地 claude-cli 慢是已知的，
    调用方多为异步 fail-silent，晚到就晚到）。通道不可用时抛 RuntimeError。
    json_mode：要 JSON 的调用点传 True，见 _run_openai（只影响 openai 兼容通道）。"""
    ...
    return _run_openai(_effective_config(), prompt, timeout=timeout, json_mode=json_mode)


def ingredient_info(name: str) -> dict:
    out = ask(ING_PROMPT.format(name=name.strip()[:20]), timeout=120, json_mode=True)
...
def _run_openai(cfg: dict, prompt: str, timeout: int = 120, json_mode: bool = False) -> str:
    """json_mode=True 时开 JSON mode：DeepSeek 支持 response_format={"type":"json_object"}
    （不支持 json_schema，会返回 unavailable），要求是提示词里出现 json 字样并给出目标格式
    示例——本文件几个提示词都写了「只输出一个 JSON…」加完整示例，满足条件。
    这是在调用方的正则抠 JSON 之前多一道保险，不是替代（CLI 通道输出仍会带前后缀）。"""
    key = os.environ.get(cfg.get("api_key_env", ""), "")
    if not key:
        raise RuntimeError(f"环境变量 {cfg.get('api_key_env')} 未设置")
    body = {"model": cfg["model"], "messages": [{"role": "user", "content": prompt}], "temperature": 0.2}
    if json_mode:
