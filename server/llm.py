...
def ask(prompt: str, timeout: int = 60) -> str:
    """按生效通道问一句，返回模型原始文本输出。
    ...
    return _run_openai(_effective_config(), prompt, timeout=timeout)


def ingredient_info(name: str) -> dict:
    out = ask(ING_PROMPT.format(name=name.strip()[:20]), timeout=120)
...
def _run_openai(cfg: dict, prompt: str, timeout: int = 120) -> str:
