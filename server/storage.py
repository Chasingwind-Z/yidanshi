...
_ID_RE = re.compile(r"[A-Za-z0-9_-]+")  # 合法菜谱 id/文件名：字母数字下划线连字符，杜绝路径穿越


def coerce_grams(v) -> int | None:
    """克重规范化：数字或纯数字字符串都收（"50"→50），负/零/非数字一律 None。"""
    try:
        g = float(v)
    except (TypeError, ValueError):
        return None
    return round(g) if g > 0 else None
...
def save_recipe(r: dict) -> dict:
    with _lock:
        if not r.get("id"):
            r["id"] = slugify(r["name"])
        elif not _ID_RE.fullmatch(str(r["id"])):
            # 显式 id 只能是合法 slug；含 / .. . 等一律拒（防越目录写 .md / 覆盖他人菜谱）
